...
def load_page_content(page_filename, page_title, fallback_message):
    ...
    try:
        ...
        # Try different case variations for cross-platform compatibility
        possible_files = [
            page_file,
            page_file.replace('.py', '.py'),  # exact
            os.path.join(current_dir, 'pages', page_filename.lower()),  # lowercase
            os.path.join(current_dir, 'pages', page_filename.capitalize()),  # capitalize
        ]
        ...
        for file_path in possible_files:
            if os.path.exists(file_path):
                try:
                    ...
                    st.warning(f"Could not read {file_path}: {read_error}")
                    continue
        
        if page_content:
            # Remove page config calls and execute
            page_content = page_content.replace('st.set_page_config(', '# st.set_page_config(')
            exec(page_content)
        else:
            st.error(f"Page file not found. Tried: {page_filename}")
            st.info(fallback_message)
            # List available files for debugging
            try:
                available_files = os.listdir(os.path.join(current_dir, 'pages'))
                st.write("Available files:", [f for f in available_files if f.endswith('.py')])
            except:
                pass
