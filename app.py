...
def load_page_content(page_filename, page_title, fallback_message):
    ...
    try:
        ...
        st.write("🔍 Debug Info:")
        st.write(f"Current script dir: {current_dir}")
        st.write(f"Working directory: {os.getcwd()}")
        
        # Try multiple approaches to find the pages directory
        possible_pages_dirs = [
        ...
        pages_dir = None
        for pd in possible_pages_dirs:
            abs_path = os.path.abspath(pd)
            st.write(f"Checking: {abs_path} - Exists: {os.path.exists(abs_path)}")
            if os.path.exists(abs_path):
                pages_dir = abs_path
                break
        
        # Also try to list what IS in the current directory
        try:
            current_contents = os.listdir(current_dir)
            st.write(f"📁 Contents of script directory: {sorted(current_contents)}")
        except Exception as e:
            st.write(f"Could not list script directory: {e}")
        
        try:
            cwd_contents = os.listdir(os.getcwd())
            st.write(f"📁 Contents of working directory: {sorted(cwd_contents)}")
        except Exception as e:
            st.write(f"Could not list working directory: {e}")
        
        if not pages_dir:
            st.error("❌ Pages directory not found in any location")
            st.info(fallback_message)
            return
        
        st.success(f"✅ Found pages directory: {pages_dir}")
        
        # Try to find the specific file
        base_name = page_filename.replace('.py', '')
        possible_files = [
            f"{base_name}.py",
            f"{base_name.lower()}.py", 
            f"{base_name.capitalize()}.py"
        ]
        
        content = None
        found_file = None
        
        # List what's actually in the pages directory
        try:
            pages_contents = os.listdir(pages_dir)
            st.write(f"📄 Files in pages directory: {sorted([f for f in pages_contents if f.endswith('.py')])}")
        except Exception as e:
            st.write(f"Could not list pages directory: {e}")
        
        for filename in possible_files:
            full_path = os.path.join(pages_dir, filename)
            st.write(f"Trying: {full_path} - Exists: {os.path.exists(full_path)}")
            if os.path.exists(full_path):
                try:
                    with open(full_path, 'r', encoding='utf-8') as f:
                        content = f.read()
                    found_file = filename
                    st.success(f"✅ Successfully loaded: {found_file}")
                    break
                except Exception as e:
        ...
        st.warning(f"❌ Could not load {page_filename}")
        st.info(fallback_message)
        
    except Exception as e:
        st.error(f"Error in loader: {str(e)}")
        import traceback
        st.code(traceback.format_exc())
        st.info(fallback_message)
