...
def load_page_content(page_filename, page_title, fallback_message):
    """Helper function to load page content with proper error handling"""
    try:
        import os
        
        # Get absolute path to the page file
        current_dir = os.path.dirname(os.path.abspath(__file__))
        page_file = os.path.join(current_dir, 'pages', page_filename)
        ...
        if os.path.exists(page_file):
            with open(page_file, 'r', encoding='utf-8') as f:
                page_content = f.read()
            # Remove page config calls
            page_content = page_content.replace('st.set_page_config(', '# st.set_page_config(')
            exec(page_content)
        else:
            st.error(f"Page file not found at: {page_file}")
            st.info(fallback_message)
        
    except Exception as e:
        st.error(f"Error loading {page_filename}: {str(e)}")
        st.info(fallback_message)
