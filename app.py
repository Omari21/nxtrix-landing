...
def load_embedded_payment_page():
    ...
    try:
        ...
        with tab1:
            ...
            with col_solo:
                founders_price = "$59" if billing_type == "Monthly" else "$708"
                ...
                <div style="border: 2px solid #4CAF50; padding: 20px; border-radius: 15px; text-align: center;">
                    <h3>🚀 Solo</h3>
                    <div style="text-decoration: line-through; color: #999; font-size: 14px;">{regular_price}{billing_cycle}</div>
                    <h2 style="color: #4CAF50;">{founders_price}{billing_cycle}</h2>
                    <p style="background: #4CAF50; color: white; padding: 5px; border-radius: 10px; margin: 10px 0;">FOUNDERS PRICE</p>
                    <hr>
                    <p style="text-align: left;">✅ Up to 100 leads/month</p>
                    <p style="text-align: left;">✅ Basic CRM & pipeline</p>
                    <p style="text-align: left;">✅ Email & SMS automation</p>
            ...
            with col_team:
                founders_price = "$89" if billing_type == "Monthly" else "$1,068"
                ...
                <div style="border: 3px solid #2196F3; padding: 20px; border-radius: 15px; text-align: center; background: #f3f8ff;">
                    <h3>⚡ Team</h3>
                    <div style="text-decoration: line-through; color: #999; font-size: 14px;">{regular_price}{billing_cycle}</div>
                    <h2 style="color: #2196F3;">{founders_price}{billing_cycle}</h2>
            ...
            with col_business:
                founders_price = "$149" if billing_type == "Monthly" else "$1,788"
                ...
                <div style="border: 2px solid #FF9800; padding: 20px; border-radius: 15px; text-align: center;">
                    <h3>🏢 Business</h3>
                    <div style="text-decoration: line-through; color: #999; font-size: 14px;">{regular_price}{billing_cycle}</div>
                    <h2 style="color: #FF9800;">{founders_price}{billing_cycle}</h2>
        ...
        with tab5:
            ...
            if current_plan == 'Free':
                ...
                with col_upgrade1:
                    if st.button("🚀 Upgrade to Team ($97/month)", type="primary"):
                        st.session_state.show_payment_form = 'Team'
                        st.rerun()
                
                with col_upgrade2:
                    if st.button("🏢 Upgrade to Business ($297/month)"):
                        st.session_state.show_payment_form = 'Business'
                        st.rerun()
            ...
            with add_tab1:
                ...
                with st.form("comprehensive_seller_lead_form"):
                    ...
                    with col_basic2:
                        ...
                        bathrooms = st.text_input("Bathrooms", placeholder="2")
        ...
        with tab4:
            ...
            for i, template in enumerate(templates):
                with cols[i % 2]:
                    ...
                    with st.container():
                        st.markdown(f"""
                        <div style="border: 2px solid #e5e7eb; padding: 1.5rem; border-radius: 15px; margin: 1rem 0; background: #f9fafb;">
                            <h4>📋 {template['name']}</h4>
                            <p><strong>Category:</strong> {template['category']}</p>
                            <p><strong>Description:</strong> {template['description']}</p>
                            <p><strong>Trigger:</strong> {template['trigger']}</p>
                            <p><strong>Setup Time:</strong> {template['setup_time']}</p>
                            <p><strong>Difficulty:</strong> {difficulty_colors[template['difficulty']]} {template['difficulty']}</p>
                        </div>
                        """, unsafe_allow_html=True)
