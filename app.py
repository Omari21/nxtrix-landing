...
def load_page_content(page_filename, page_title, fallback_message):
    ...
    try:
        ...
        # Primary: Try pages directory
        pages_dir = os.path.join(current_dir, 'pages')
        
        # Fallback: Try root directory if pages not found
        if not os.path.exists(pages_dir):
            pages_dir = current_dir
            st.info("📁 Loading from root directory (fallback mode)")
        
        # Try to find the specific file
        base_name = page_filename.replace('.py', '')
        possible_files = [
            f"{base_name}.py",                    # Standard: leads.py
            f"{base_name.lower()}.py",            # Lowercase: leads.py
            f"{base_name.capitalize()}.py",       # Capitalized: Leads.py
            f"{base_name}_backup.py"              # Fallback: leads_backup.py
        ]
        ...
        if content and found_file:
            ...
            # Show basic info if page not found
            ...
            try:
                ...
                if available_files:
                    st.write("📄 Available pages:", ", ".join(sorted(available_files)[:10]))
            except Exception:
                pass
