...
def load_page_content(page_filename, page_title, fallback_message):
    ...
    try:
        ...
        # Try to find the file with different case variations
        possible_files = [
            page_filename,
            page_filename.lower(),
            page_filename.capitalize(),
        ]
        ...
        for filename in possible_files:
            ...
            if os.path.exists(full_path):
                try:
                    ...
                    st.warning(f"Could not read {filename}: {read_error}")
                    continue
        ...
        if file_found and content:
            try:
                ...
                exec_globals = {
                    'st': st,
                    '__name__': '__main__',
                    '__file__': os.path.join(pages_dir, used_filename)
                }
                exec(content, exec_globals)
            except Exception as exec_error:
                st.error(f"Error executing {used_filename}: {exec_error}")
                st.info("Loading basic fallback version...")
            ...
            try:
                ...
                st.write("📁 Available page files:", sorted(available_files[:15]))
            except Exception as e:
                st.write(f"Could not list files: {e}")
        
    except Exception as e:
        st.error(f"Error in loader for {page_filename}: {str(e)}")
        st.info(fallback_message)
