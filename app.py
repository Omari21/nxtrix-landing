...
def load_page_content(page_filename, page_title, fallback_message):
    ...
    try:
        ...
        # List all possible locations and file variations
        search_paths = [
            ...
            (os.path.join(current_dir, 'pages'), f"{page_filename.replace('.py', '').lower()}.py"),
            (os.path.join(current_dir, 'pages'), f"{page_filename.replace('.py', '').capitalize()}.py"),
        ]
        ...
        # Try each path/file combination
        ...
        if content and found_file:
            try:
                ...
                exec_globals = {
                    'st': st,
                    '__name__': '__main__',
                    '__file__': os.path.join(found_path, found_file)
                }
            ...
            st.warning(f"Could not find {page_filename}")
            
            # Show what files are available for debugging
            try:
                # Check root directory
                root_files = [f for f in os.listdir(current_dir) if f.endswith('.py')]
                st.write(f"📄 Root directory files: {sorted([f for f in root_files if 'backup' in f or page_filename.replace('.py', '') in f])}")
                
                # Check pages directory if it exists
                pages_dir = os.path.join(current_dir, 'pages')
                if os.path.exists(pages_dir):
                    pages_files = [f for f in os.listdir(pages_dir) if f.endswith('.py')]
                    st.write(f"📁 Pages directory files: {sorted(pages_files[:10])}")
                else:
                    st.write("📁 Pages directory: Not found")
