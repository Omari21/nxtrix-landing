...
def load_page_content(page_filename, page_title, fallback_message):
    ...
    try:
        ...
        # Try multiple approaches to find the page files
        # 1. First try the pages directory
        possible_pages_dirs = [
        ...
        # 2. Also try root directory as fallback
        possible_root_dirs = [
            current_dir,
            os.getcwd(),
            '.'
        ...
        # If pages directory not found, try root directory with backup files
        if not pages_dir:
            st.warning("Pages directory not found, trying root directory fallbacks...")
            for rd in possible_root_dirs:
                abs_path = os.path.abspath(rd)
                if os.path.exists(abs_path):
                    pages_dir = abs_path
                    break
        
        if not pages_dir:
            st.error("❌ No suitable directory found for page files")
        ...
        possible_files = [
            f"{base_name}.py",                    # pages/leads.py
            f"{base_name.lower()}.py",            # pages/leads.py (lowercase)
            f"{base_name.capitalize()}.py",       # pages/Leads.py (capitalized)
            f"{base_name}_backup.py"              # leads_backup.py (root fallback)
        ]
            ...
            if os.path.exists(full_path):
                try:
                    with open(full_path, 'r', encoding='utf-8') as f:
                        content = f.read()
                    found_file = filename
                    st.success(f"✅ Successfully loaded: {found_file} from {pages_dir}")
                    break
                except Exception as e:
        ...
        try:
            available_files = [f for f in os.listdir(pages_dir) if f.endswith('.py')]
            st.write(f"📄 Available .py files in {pages_dir}: {sorted(available_files)}")
        except Exception as e:
            st.write(f"Could not list files in {pages_dir}: {e}")
